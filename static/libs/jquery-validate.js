(function(factory) {
    ...
    var RULES = {
        ...
        onlyNum: function(value, errorMsg) {
            //数字包含小数
            if (!/^[0-9]+([.][0-9]+){0,1}$/.test(value) || value.length<6) {
                return errorMsg;
            }
        },
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
        ...
        $msg.remove();
        if(_current == 'error'){
            $context.removeClass('success tip error').addClass(_current+' '+_other).append('<span class="valid_message"><img style="margin-top:0" src="/static/images/accounterror.png">' + msg + '</span>');
        }else{
            $context.removeClass('success tip error').addClass(_current+' '+_other).append('<span class="valid_message"><img style="margin-top:0" src="/static/images/info3.png">' + msg + '</span>');
